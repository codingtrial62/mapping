...
def get_all():
    ...
    for i in range(wdf.shape[0]):
        ...
        if wdf.loc[i, 'geometry'].geom_type == 'Point':
            ...
            if wdf.loc[i, 'aerodrome'] == 'ltfe_Area2a_Obstacles':
                polylines.append({'latlngs':chunks3(wdf.loc[i, 'coordinate'].replace(',', '.').split(' '), 2), 'popup': f"Elevation: {wdf.loc[i, 'elevation']} FT  Type: {wdf.loc[i, 'obstacle_type']} Coordinates(..N..E): {chunks3(wdf.loc[i, 'coordinate'].replace(',', '.').split(' '), 2)}"})
            elif wdf.loc[i, 'aerodrome'] != 'ltfe_Area2a_Obstacles':
                polylines.append({'latlngs':chunks2(wdf.loc[i, 'coordinate'].replace(',', '.').split(' '), 2), 'popup': f"Elevation: {wdf.loc[i, 'elevation']} FT  Type: {wdf.loc[i, 'obstacle_type']} Coordinates(..N..E): {chunks2(wdf.loc[i, 'coordinate'].replace(',', '.').split(' '), 2)}"})
        elif wdf.loc[i, 'geometry'].geom_type == 'MultiPolygon':
            polygons.append({'latlngs':chunks2(wdf.loc[i, 'coordinate'].replace(',', '.').split(' '), 2), 'popup': f"Elevation: {wdf.loc[i, 'elevation']} FT  Type: {wdf.loc[i, 'obstacle_type']}  Coordinates(..N..E): {chunks2(wdf.loc[i, 'coordinate'].replace(',', '.').split(' '), 2)}"})
    ...
    for e in range(ltac.shape[0]):
        ...
        if ltac.loc[e, 'geometry'].geom_type == 'Point':
            ...
            polylines.append({'latlngs':chunks2(ltac.loc[e, 'Coordinate'].replace(',', '.').split(' '), 2), 'popup': f"Elevation: {ltac.loc[e, 'Elevation']} FT  Type: {ltac.loc[e, 'Obstacle_Type']}  Coordinates(..N..E): {chunks2(ltac.loc[e, 'Coordinate'].replace(',', '.').split(' '), 2)}"})

        elif ltac.loc[e, 'geometry'].geom_type == 'Polygon':
            polygons.append({'latlngs':chunks2(ltac.loc[e, 'Coordinate'].replace(',', '.').split(' '), 2), 'popup': f"Elevation: {ltac.loc[e, 'Elevation']} FT  Type: {ltac.loc[e, 'Obstacle_Type']}  Coordinates(..N..E): {chunks2(ltac.loc[e, 'Coordinate'].replace(',', '.').split(' '), 2)}"})
    ...
    for t in range(gdf.shape[0]):
        ...
        if gdf.loc[t, 'geometry'].geom_type == 'Point':
            ...
            polylines.append({'latlngs':chunks2(gdf.loc[t, 'coordinate'].replace(',', '.').split(' '), 2), 'popup': f"Elevation: {gdf.loc[t, 'elevation']} FT  Type: {gdf.loc[t, 'obstacle_type']}  Coordinates(..N..E): {chunks2(gdf.loc[t, 'coordinate'].replace(',', '.').split(' '), 2)}"})
    ...
    for u in range(xdf.shape[0]):
        ...
        if xdf.loc[u, 'geometry'].geom_type == 'Point':
            ...
            polylines.append({'latlngs':chunks2(xdf.loc[u, 'coordinate'].replace(',', '.').split(' '), 2), 'popup': f"Elevation: {xdf.loc[u, 'elevation']} FT  Type: {xdf.loc[u, 'type']}  Coordinates(..N..E): {chunks2(xdf.loc[u, 'coordinate'].replace(',', '.').split(' '), 2)}"})

        elif xdf.loc[u, 'geometry'].geom_type == 'MultiPolygon':
            polygons.append({'latlngs':chunks2(xdf.loc[u, 'coordinate'].replace(',', '.').split(' '), 2), 'popup': f"Elevation: {xdf.loc[u, 'elevation']} FT  Type: {xdf.loc[u, 'type']}  Coordinates(..N..E): {chunks2(xdf.loc[u, 'coordinate'].replace(',', '.').split(' '), 2)}"})
    ...
    for l in range(hdf.shape[0]):
        ...
        if hdf.loc[l, 'geometry'].geom_type == 'MultiLineString':
            ...
            if hdf.loc[l, 'aerodrome'] == 'ltfe_area_4_area_4_28r_area_4_28r_Area4_Obstacles' or hdf.loc[l, 'aerodrome'] == 'ltfe_area_4_area_4_10l_area_4_10l_Area4_Obstacles':
                polylines.append({'latlngs':chunks3(hdf.loc[l, 'coordinate'].replace(',', '.').split(' '), 2), 'popup': f"Elevation: {hdf.loc[l, 'elevation']} FT  Type: {hdf.loc[l, 'obstacle_type']}  Coordinates(..N..E): {chunks3(hdf.loc[l, 'coordinate'].replace(',', '.').split(' '), 2)}"})
            else:
                polylines.append({'latlngs':chunks2(hdf.loc[l, 'coordinate'].replace(',', '.').split(' '), 2), 'popup': f"Elevation: {hdf.loc[l, 'elevation']} FT  Type: {hdf.loc[l, 'obstacle_type']}  Coordinates(..N..E): {chunks2(hdf.loc[l, 'coordinate'].replace(',', '.').split(' '), 2)}"})
            ...
            if hdf.loc[l, 'aerodrome'] == 'ltfe_area_4_area_4_28r_area_4_28r_Area4_Obstacles':
                polygons.append({'latlngs':chunks3(hdf.loc[l, 'coordinate'].replace(',', '.').split(' '), 2), 'popup': f"Elevation: {hdf.loc[l, 'elevation']} FT  Type: {hdf.loc[l, 'obstacle_type']}  Coordinates(..N..E): {chunks3(hdf.loc[l, 'coordinate'].replace(',', '.').split(' '), 2)}"})
            else:

                polygons.append({'latlngs':chunks2(hdf.loc[l, 'coordinate'].replace(',', '.').split(' '), 2), 'popup': f"Elevation: {hdf.loc[l, 'elevation']} FT  Type: {hdf.loc[l, 'obstacle_type']}  Coordinates(..N..E): {chunks2(hdf.loc[l, 'coordinate'].replace(',', '.').split(' '), 2)}"})
    ...
    for i in range(gdf.shape[0]):
        ...
        if gdf.loc[i, 'geometry'].geom_type == 'Point':
            ...
            if gdf.loc[i, 'aerodrome'] == 'ltfe_Area2a_Obstacles':
                polylines.append({'latlngs':chunks3(gdf.loc[i, 'coordinate'].replace(',', '.').split(' '), 2), 'popup': f"Elevation: {gdf.loc[i, 'elevation']} FT  Type: {gdf.loc[i, 'obstacle_type']} Coordinates(..N..E): {chunks3(gdf.loc[i, 'coordinate'].replace(',', '.').split(' '), 2)}"})
            else:
                polylines.append({'latlngs':chunks2(gdf.loc[i, 'coordinate'].replace(',', '.').split(' '), 2), 'popup': f"Elevation: {gdf.loc[i, 'elevation']} FT  Type: {gdf.loc[i, 'obstacle_type']} Coordinates(..N..E): {chunks2(gdf.loc[i, 'coordinate'].replace(',', '.').split(' '), 2)}"})
        elif gdf.loc[i, 'geometry'].geom_type == 'MultiPolygon':
            polygons.append({'latlngs':chunks2(gdf.loc[i, 'coordinate'].replace(',', '.').split(' '), 2), 'popup': f"Elevation: {gdf.loc[i, 'elevation']} FT  Type: {gdf.loc[i, 'obstacle_type']}  Coordinates(..N..E): {chunks2(gdf.loc[i, 'coordinate'].replace(',', '.').split(' '), 2)}"})
    return jsonify({'points': point, 'polylines': polylines, 'polygons': polygons})
